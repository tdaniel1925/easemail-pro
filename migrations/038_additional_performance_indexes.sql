...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_emails_account_folder_active
ON emails(account_id, folder)
WHERE is_trashed = false AND is_archived = false;

-- ✅ Index for faster unread count queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_emails_account_read_status
ON emails(account_id, is_read)
WHERE is_trashed = false AND is_archived = false;

-- ✅ GIN index for faster JSON array searches in folders field
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_emails_folders_gin
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_user_active
ON contacts_v4(user_id, is_deleted)
WHERE is_deleted = false;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_display_name
ON contacts_v4(display_name)
WHERE is_deleted = false;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_company
ON contacts_v4(company_name)
WHERE is_deleted = false;

-- ✅ GIN indexes for JSON array searches in contacts
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_emails_gin
ON contacts_v4 USING GIN (emails);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_phone_gin
ON contacts_v4 USING GIN (phone_numbers);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_groups_gin
ON contacts_v4 USING GIN (groups);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_v4_tags_gin
ON contacts_v4 USING GIN (tags);

-- ✅ Email drafts - improve draft count queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_drafts_account_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_webhook_events_nylas_id
