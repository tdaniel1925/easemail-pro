...
export default function FolderNav({
  ...
  return (
    <div className="w-[var(--sidebar-width)] border-r border-border bg-background flex flex-col" role="complementary" aria-label="Email navigation sidebar">
      {/* Header */}
      <div className="p-4 border-b border-border">
        ...
        {accounts.length > 0 && (
          <Select value={selectedAccount || undefined} onValueChange={onAccountChange}>
            <SelectTrigger className="w-full" aria-label="Select email account">
      ...
      <ScrollArea className="flex-1 p-2">
        <nav aria-label="Email folders" className="space-y-1">
          {folders.map(folder => {
        ...
        <div className="mt-6 px-2">
          <h3 id="labels-heading" className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
            Labels
          </h3>
          <nav aria-labelledby="labels-heading" className="space-y-1">
            <Button
              variant="ghost"
              className="w-full justify-start text-sm"
              disabled
              aria-label="No labels available"
            >
              <Tag className="h-4 w-4 mr-3 text-muted-foreground" aria-hidden="true" />
      ...
      <div className="border-t border-border p-4">
        ...
        <div className="mt-3 text-xs text-muted-foreground" role="status" aria-label="Storage usage: 2.5 GB of 15 GB used">
          <div className="flex justify-between mb-1">
            <span>Storage</span>
            <span>2.5 GB / 15 GB</span>
          </div>
          <div className="w-full bg-muted rounded-full h-1" role="progressbar" aria-valuenow={16.7} aria-valuemin={0} aria-valuemax={100} aria-label="Storage usage">
            <div
