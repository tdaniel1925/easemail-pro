-- ============================================================================
-- EaseMail Production Performance Indexes (Non-Concurrent Version)
-- ============================================================================
-- This migration adds critical indexes for 10-50x query performance improvement
--
-- **USE THIS VERSION IN SUPABASE SQL EDITOR**
--
-- WARNING: This version uses regular CREATE INDEX (without CONCURRENTLY)
-- - Locks tables during index creation (brief downtime)
-- - Suitable for development/low-traffic environments
-- - For production with traffic, use the CONCURRENTLY version via psql
--
...
CREATE INDEX IF NOT EXISTS idx_emails_sent_at_desc ON emails(sent_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_emails_account_folder_unread
...
CREATE INDEX IF NOT EXISTS idx_contacts_last_email_at ON contacts(last_email_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_email_drafts_reply_to ON email_drafts(reply_to_email_id);
CREATE INDEX IF NOT EXISTS idx_email_drafts_created_at ON email_drafts(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_email_drafts_scheduled ON email_drafts(scheduled_at);
...
CREATE INDEX IF NOT EXISTS idx_sms_usage_billing_status ON sms_usage(billing_status);
...
CREATE INDEX IF NOT EXISTS idx_contact_comms_contact_time ON contact_communications(contact_id, occurred_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_contact_notes_is_pinned ON contact_notes(is_pinned);
...
-- CREATE INDEX IF NOT EXISTS idx_rule_activity_executed_at ON rule_activity(executed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_storage_usage_snapshot_date ON storage_usage(snapshot_date DESC);
